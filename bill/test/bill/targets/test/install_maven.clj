...
(deftest test-parse-args
  ...
  (is (= (parse-args ["-g" group "-a" artifact "-v" version "--algorithm" md5-algorithm])
          [{ :group group :artifact artifact :version version :algorithm md5-algorithm } [test-jar-path] usage]))
...
(deftest test-update-repository
  (let [options { :artifact artifact :version version :algorithm algorithm }
        args [test-jar-path]
